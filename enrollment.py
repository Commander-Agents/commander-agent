...
class EnrollmentManager:
    ...
    def enroll(self):
        ...
        print("[Enrollment] Attempting to enroll...")
        try:
            ...
            if data["success"]:
                print("[Enrollment] Enrollment from server is OK")
                print("[Enrollment] Registering server public key")
                server_public_key = data["server_public_key"]
                self.crypto_utils.save_server_public_key(server_public_key)
                print("[Enrollment] Server public key registered")
                ...
                try:
                    ...
                    print("[Enrollmen] Error while decrypting HMAC key :", str(e))
            else:
                raise Exception("Error while enrolling from the server") 

        except Exception as e:
            print(f"[Enrollmen] Enrollment failed: {e}")
            sys.exit(0)
